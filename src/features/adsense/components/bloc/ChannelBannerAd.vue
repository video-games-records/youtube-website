<!-- src/features/adsense/components/bloc/ChannelBannerAd.vue -->
<template>
  <div
      v-if="shouldShow"
      class="channel-banner-ad"
...
// Computed
const adSlot = computed(() => AD_SLOTS.CHANNEL_BANNER)
...
const adStyles = computed(() => {
  const format = AD_FORMATS.CHANNEL_BANNER
  return {
...
const loadAd = async () => {
  ...
  try {
    if (adElement.value) {
      adsense.push({})
      console.debug('Channel banner ad loaded')
    }
  } catch (error) {
    console.error('Erreur chargement annonce channel banner:', error)
  }
}
