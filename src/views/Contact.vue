...
const loadComponent = async () => {
  ...
  try {
    console.log('Loading Contact for locale:', locale.value)

    let component
    switch (locale.value) {
      case 'fr':
        component = (await import('./contact/Contact.fr.vue')).default
        break
      case 'en':
      default:
        component = (await import('./contact/Contact.en.vue')).default
        break
    }
  ...
  <div>
    ...
    <div v-if="isLoading" class="flex justify-center items-center min-h-[50vh]">
      <div class="text-center">
        ...
        <p class="text-muted-foreground">Loading Contact...</p>
      </div>
    </div>
    ...
    <div v-else class="flex justify-center items-center min-h-[50vh]">
      <p class="text-muted-foreground">Failed to load Contact content</p>
    </div>
  </div>
</template>
